...
InsertionView.prototype.renderInput = function (type, parameter) {
  const self = this;
  const renderType = {
    "DRAW LIST": function () {
...
InsertionView.prototype.renderVisualize = async function ({
  type,
  sourceIndex,
  targetIndex,
}) {
  ...
  const visualize = {
    ...
    "UNCOLOR TARGET": async function () {
      source.children[0].classList.remove("done");
      await wait(600);
    },
    "PICK TARGET": async function () {
...
InsertionView.prototype.connectHandler = function (event, handler) {
  const self = this;
...
InsertionView.prototype.paintBar = function (list) {
